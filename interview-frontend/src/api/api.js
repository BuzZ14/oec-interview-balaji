...
export const addUserToPlanProceduer = async (planId, procedureId, userIds) => {
    const url = `${api_url}/PlanProcedureUser/AddUserToPlanProcedure`;
    ...
    if (!response.ok) throw new Error("Failed to assign user to the plan procedure");

    return true;
};

export const getAssignedProcedureUsers = async (planid, procedureId) => {
    const url = `${api_url}/PlanProcedureUser/GetAssignedUsersForPlanAndProcedure?planId=${planid}&procedureId=${procedureId}`;
    const response = await fetch(url, {
        method: "GET",
    });

    if (!response.ok) throw new Error("Failed to get asigned users");
